...
# Made by : Jean-Christophe Fronteddu
# Date of creation :            22 june 2021
# Date of last modification :   27 august 2021
# Author of last modification : Jean-Christophe    
# MIT license
###################################################
...
# Creating LABEL
info_lbl = Label(root, text="Please enter the directory path containing the sample tiff files")
info_lbl.grid(row=0, column=0, columnspan=2, sticky='W')
label = Label(root, text="path :", font=30)
...
# Creating BUTTONS
browse_button = Button(root, text="Browse", width=50, height=2, font=30, command=choose_directory)
browse_button.grid(row=2, column=0, columnspan=2, sticky='W')
ok_button = Button(root, text="Ok", width=10, height=2, font=30, command=ok_function)
ok_button.grid(row=3, column=0, columnspan=2, sticky='W')
cancel_button = Button(root, text="Cancel", width=10, height=2, font=30, command=cancel_function)
...
if valid:
    ...
    def calculate2dGraph(values, advanced_values, nbSlice, dimension, 
        ...
        SE_perim_radius             =advanced_values[0]            
        SE_edges_radius             =advanced_values[1]            
        SE_fills_radius             =advanced_values[2]            
        SE_large_radius             =advanced_values[3]            
        ...
        if findExternalPerimeter:
            SE_perim = cv.getStructuringElement(cv.MORPH_ELLIPSE, ksize=(SE_perim_radius, SE_perim_radius))
        ...
        if findPores:
            SE_edges = cv.getStructuringElement(cv.MORPH_ELLIPSE, ksize=(SE_edges_radius, SE_edges_radius))
            SE_edges[:, 0]=SE_edges[ 0,:]
            SE_edges[:,-1]=SE_edges[-1,:]

            SE_fills = cv.getStructuringElement(cv.MORPH_ELLIPSE, ksize=(SE_fills_radius, SE_fills_radius))
            SE_fills[:, 0]=SE_fills[ 0,:]
            SE_fills[:,-1]=SE_fills[-1,:]

            SE_large = cv.getStructuringElement(cv.MORPH_ELLIPSE, ksize=(SE_large_radius, SE_large_radius))
    ...
    def calculate3dGraph(values, advanced_values, nbSlice, dimension,
        ...
        if findExternalPerimeter:
            ...
            paddingSize=SE_perim_radius # to avoid artifacts on corners after opening
    ...
    def show2dGraph(Graph2D):
        ...
        for imSlice in range(iFirst, iLast+1):
            ...
            if findExternalPerimeter and not findPores:
                ...
                canvas = FigureCanvasTkAgg(fig_CannyDetection[imSlice-1][0], graph2_frame)
                canvas.draw()
                canvas.get_tk_widget().grid(row=0, column=1, padx=5, pady=5)
                canvas = FigureCanvasTkAgg(fig_CannyDetection[imSlice-1][1], graph2_frame)
                canvas.draw()
                canvas.get_tk_widget().grid(row=0, column=0, padx=5, pady=5)
                canvas = FigureCanvasTkAgg(fig_CannyDetection[imSlice-1][2], graph2_frame)
                canvas.draw()
                canvas.get_tk_widget().grid(row=1, column=0, padx=5, pady=5)
                canvas = FigureCanvasTkAgg(fig_CannyDetection[imSlice-1][3], graph2_frame)
                canvas.draw()
                canvas.get_tk_widget().grid(row=1, column=1, padx=5, pady=5)
                canvas = FigureCanvasTkAgg(fig_CannyDetection[imSlice-1][4], graph2_frame)
                canvas.draw()
                canvas.get_tk_widget().grid(row=2, column=0, padx=5, pady=5)
            elif not findExternalPerimeter and findPores:
                graph_canvas.create_window((0,0), window=graph2_frame, anchor='nw')                    
                canvas = FigureCanvasTkAgg(fig_ContourDetection[imSlice-1][0], graph2_frame)
                canvas.draw()
                canvas.get_tk_widget().grid(row=0, column=0, padx=5, pady=5)
                canvas = FigureCanvasTkAgg(fig_ContourDetection[imSlice-1][1], graph2_frame)
                canvas.draw() 
                canvas.get_tk_widget().grid(row=0, column=1, padx=5, pady=5)
            elif findExternalPerimeter and findPores:
                graph_canvas.create_window((0,0), window=graph2_frame, anchor='nw')
    ...
    def show3dGraph(fig_volumetricProcessing):        
        ...
        for imSlice in range(iFirst, iLast+1):
            if findExternalPerimeter and not findPores:
                # Creating the new window to display with the scroll
                graph_canvas.create_window((0,0), window=graph2_frame, anchor='nw')
                canvas = FigureCanvasTkAgg(fig_volumetricProcessing[0][imSlice-1], graph2_frame)
                canvas.draw()
                canvas.get_tk_widget().grid(row=0, column=0, padx=5, pady=5)
                canvas = FigureCanvasTkAgg(fig_volumetricProcessing[1][imSlice-1], graph2_frame)
                canvas.draw()
                canvas.get_tk_widget().grid(row=0, column=1, padx=5, pady=5)
            elif not findExternalPerimeter and findPores:
                canvas = FigureCanvasTkAgg(fig_volumetricProcessing[2][imSlice-1], graph2_frame)
                canvas.draw()
                canvas.get_tk_widget().grid(row=0, column=0, padx=5, pady=5)
                canvas = FigureCanvasTkAgg(fig_volumetricProcessing[3][imSlice-1], graph2_frame)
                canvas.draw()
                canvas.get_tk_widget().grid(row=0, column=1, padx=5, pady=5)                 
            elif findExternalPerimeter and findPores:
                graph_canvas.create_window((0,0), window=graph2_frame, anchor='nw')
    ...
    def verify_number():
        ...
        try:
            ...
            int(SE_perim_radius_entry.get())
            int(SE_edges_radius_entry.get())
            int(SE_fills_radius_entry.get())
            int(SE_large_radius_entry.get())
    ...
    def verify_entries(launch=False): 
        ...
        values=[int(ThresholdPerimeter_entry.get()), 
                ...
                int(PoreLow_entry.get()),
                float(PoreSigma_entry.get())]
        advanced_values=[int(SE_perim_radius_entry.get()),
                        int(SE_edges_radius_entry.get()),
                        int(SE_edges_radius_entry.get()),
                        int(SE_large_radius_entry.get()),
                        int(SE_perim_3DOpening_radius_entry.get()),
        ...
        if values[0] < 0 or values[0] > 255:
            messagebox.showerror("Error", "Treshold Perimeter : %d You have to enter a number between [0, 255]" %(values[0]))
            valid = False
            ...
            messagebox.showerror("Error", "Perimeter : %d You have to enter a number between [0, 10]" %(values[3]))
            valid = False
            ...
            messagebox.showerror("Error", "Pore diameter : %d You have to enter a number between [0, 10]" %(values[6]))
            valid = False
        elif advanced_values[0] < 0 or advanced_values[0] > 100:
            messagebox.showerror("Error", "SE perimeter radius : %d You have to enter a number between [0, 100]" %(advanced_values[0]))
            valid = False
        elif advanced_values[1] < 0 or advanced_values[1] > 100:
            messagebox.showerror("Error", "SE edges radius : %d You have to enter a number between [0, 100]" %(advanced_values[1]))
            valid = False
        elif advanced_values[2] < 0 or advanced_values[2] > 100:
            messagebox.showerror("Error", "SE fills radius : %d You have to enter a number between [0, 100]" %(advanced_values[2]))
            valid = False
        elif advanced_values[3] < 0 or advanced_values[3] > 100:
            messagebox.showerror("Error", "SE large radius : %d You have to enter a number between [0, 100]" %(advanced_values[3]))
            valid = False
    ...
    def info_request_command(infoRequest):
        if infoRequest == 1:
            messagebox.showinfo("Information", "Treshold Perimeter :")
        elif infoRequest == 2:
            messagebox.showinfo("Information", "Perimeter High :")
        elif infoRequest == 3:
            messagebox.showinfo("Information", "Perimeter Low :")
        elif infoRequest == 4:
            messagebox.showinfo("Information", "Perimeter Sigma :")
        elif infoRequest == 5:
            messagebox.showinfo("Information", "Pores High :")
        elif infoRequest == 6:
            messagebox.showinfo("Information", "Pores Low :")
        elif infoRequest == 7:
            messagebox.showinfo("Information", "Pores Sigma :")
        elif infoRequest == 8:
            messagebox.showinfo("Information", "SE perimeter radius :")
        elif infoRequest == 9:
    ...
    PerimeterHigh_lbl = Label(root, text="Perimeter high")
    PerimeterHigh_lbl.grid(row=1, column=0, sticky='W')
    PerimeterLow_lbl = Label(root, text="Perimeter low")
    PerimeterLow_lbl.grid(row=2, column=0, sticky='W')
    PerimeterSigma_lbl = Label(root, text="Perimeter sigma")
    PerimeterSigma_lbl.grid(row=3, column=0, sticky='W')
    PoreHigh_lbl = Label(root, text="Pore high")
    PoreHigh_lbl.grid(row=4, column=0, sticky='W')
    PoreLow_lbl = Label(root, text="Pore low")
    PoreLow_lbl.grid(row=5, column=0, sticky='W')
    PoreSigma_lbl = Label(root, text="Pore diameter")
    PoreSigma_lbl.grid(row=6, column=0, sticky='W')

    # Creating LABEL to hold information about the variable
    TP_info_lbl = Label(root, text="Please insert a value between [0, 255]")
    TP_info_lbl.grid(row=0, column=2, sticky='W')
    PeriH_info_lbl = Label(root, text="Please insert a value between [0, 255]")
    PeriH_info_lbl.grid(row=1, column=2, sticky='W')
    PeriL_info_lbl = Label(root, text="Please insert a value between [0, 255]")
    PeriL_info_lbl.grid(row=2, column=2, sticky='W')
    PeriS_info_lbl = Label(root, text="Please insert a value between [0, 10]")
    PeriS_info_lbl.grid(row=3, column=2, sticky='W')
    PoreH_info_lbl = Label(root, text="Please insert a value between [0, 255]")
    PoreH_info_lbl.grid(row=4, column=2, sticky='W')
    PoreL_info_lbl = Label(root, text="Please insert a value between [0, 255]")
    PoreL_info_lbl.grid(row=5, column=2, sticky='W')
    PoreS_info_lbl = Label(root, text="Please insert a value between [0, 10]")
    ...
    SE_perim_radius_lbl = Label(Advanced_Parameter_frame, text="SE perimeter radius")
    SE_perim_radius_lbl.grid(row=0, column=0, sticky='W')
    SE_edges_radius_lbl = Label(Advanced_Parameter_frame, text="SE edges radius")
    SE_edges_radius_lbl.grid(row=1, column=0, sticky='W')
    SE_fills_radius_lbl = Label(Advanced_Parameter_frame, text="SE fills radius")
    SE_fills_radius_lbl.grid(row=2, column=0, sticky='W')
    SE_large_radius_lbl = Label(Advanced_Parameter_frame, text="SE large radius")
    SE_large_radius_lbl.grid(row=3, column=0, sticky='W')
    ...
    SE_perim_radius_info_lbl = Label(Advanced_Parameter_frame, text="Please insert a value between [0, 255]")
    SE_perim_radius_info_lbl.grid(row=0, column=2, sticky='W')
    SE_edges_radius_info_lbl = Label(Advanced_Parameter_frame, text="Please insert a value between [0, 255]")
    SE_edges_radius_info_lbl.grid(row=1, column=2, sticky='W')
    SE_fills_radius_info_lbl = Label(Advanced_Parameter_frame, text="Please insert a value between [0, 255]")
    SE_fills_radius_info_lbl.grid(row=2, column=2, sticky='W')
    SE_large_radius_info_lbl = Label(Advanced_Parameter_frame, text="Please insert a value between [0, 255]")
    SE_large_radius_info_lbl.grid(row=3, column=2, sticky='W')
    SE_perim_3DOpening_radius_info_lbl = Label(Advanced_Parameter_frame, text="Please insert a value between [0, 255]")
    SE_perim_3DOpening_radius_info_lbl.grid(row=4, column=2, sticky='W')
    SE_pores3d_radiusOpening_info_lbl = Label(Advanced_Parameter_frame, text="Please insert a value between [0, 255]")
    SE_pores3d_radiusOpening_info_lbl.grid(row=5, column=2, sticky='W')
    SE_pores3d_radiusClosing_info_lbl = Label(Advanced_Parameter_frame, text="Please insert a value between [0, 255]")
    ...
    ThresholdPerimeter_entry.insert(0, "50")
    PerimeterHigh_entry.insert(0, "50")
    PerimeterLow_entry.insert(0, "30")
    PerimeterSigma_entry.insert(0, "1")
    PoreHigh_entry.insert(0, "180")
    PoreLow_entry.insert(0, "60")
    PoreSigma_entry.insert(0, "3")

    # Creating ENTRY for Advance parameters
    SE_perim_radius_entry = Entry(Advanced_Parameter_frame, width=7)
    SE_perim_radius_entry.grid(row=0, column=1, padx=5, pady=5, sticky='W')
    SE_edges_radius_entry = Entry(Advanced_Parameter_frame, width=7)
    SE_edges_radius_entry.grid(row=1, column=1, padx=5, pady=5, sticky='W')
    SE_fills_radius_entry = Entry(Advanced_Parameter_frame, width=7)
    SE_fills_radius_entry.grid(row=2, column=1, padx=5, pady=5, sticky='W')
    SE_large_radius_entry = Entry(Advanced_Parameter_frame, width=7)
    SE_large_radius_entry.grid(row=3, column=1, padx=5, pady=5, sticky='W')
    ...
    SE_perim_radius_entry.insert(0, "15")
    SE_edges_radius_entry.insert(0, "9")
    SE_fills_radius_entry.insert(0, "5")
    SE_large_radius_entry.insert(0, "15")
    SE_perim_3DOpening_radius_entry.insert(0, "3")
    SE_pores3d_radiusOpening_entry.insert(0, "1")
    SE_pores3d_radiusClosing_entry.insert(0, "3")
    ...
    validateAll_btn = Button(Button_frame, text="Validate All", width=12, height=2, command=validate_AllParam_command)
