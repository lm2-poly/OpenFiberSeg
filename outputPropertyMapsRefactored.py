...
def outputPropertyMap(commonPath,parallelHandle=True,randomizeFiberMap=False,croppedFiles=False):
    ...
    # #necessary to flip before constructing vtk, so that coordinates coincide with tiff files
    # V_fiberMap_flipped=np.flip(V_fiberMap,axis=1)
    ...
    for fiberID,fibData in processedFibers.items():
        ...
        marker_to_lengthLUT[fiberID]=fibData["length"]
